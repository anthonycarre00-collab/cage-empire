...
def _progress_training_camp(conn, camp_id, fighter_id, fatigue, morale,
    ...
    if new_risk > 80:
        ...
        # is a placeholder.
        # NEWS-FINANCE-GYM-LEGACY Issue 6.1 — removed the inline
        # training-injury news write (was a duplicate of the richer
        # news.generate_camp_injury_news subscriber on CAMP_INJURY).
        # The event-bus publish below (CAMP_INJURY) drives the only
        # news item for this case, halving training-topic news volume.
...
def _complete_training_camp(conn, camp_id, fighter_id, camp_focus,
    ...
    # NEWS-FINANCE-GYM-LEGACY Issue 6.1 — removed the inline
    # completion news write. It was a duplicate of the richer
    # news.generate_camp_news subscriber on CAMP_COMPLETED (which
    # fires below at the bus.publish call). Inline write was the
    # primary source of training-topic news spam (~338 items/year);
    # the bus-driven item is voice-compliant and importance-tiered.
