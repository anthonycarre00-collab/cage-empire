...
Visual spec (UI_REDESIGN_VISUAL_PLAN §5.10 + UI-REDESIGN-DASH-V2 Fix #6):

  +-----------+-------------+-----------+--------+--------+-----------+
  | Variant   | bg          | text      | border | radius | padding   |
  +-----------+-------------+-----------+--------+--------+-----------+
  | Primary   | gold        | text_on_gold | none | 10px  | 10×20    |
  | Secondary | transparent | text_primary | border_subtle 1px | 10px | 10×20 |
  | Danger    | crimson     | text_on_crimson | none | 10px | 10×20   |
  | Ghost     | transparent | text_secondary | none | 10px  | 8×12     |
  +-----------+-------------+-----------+--------+--------+-----------+

  Fix #6 (UI-REDESIGN-DASH-V2): corner_radius bumped from 4 → 10 for
  a more rounded, polished feel. Mirrored in the CTk theme JSON.
...
# Fix #6 (UI-REDESIGN-DASH-V2): rounded button radius.
# Was 4 (too square — felt "default CTk"). Bumped to 10 for the
# rounded, polished feel the user asked for.
_BUTTON_CORNER_RADIUS = 10
...
class Button(ctk.CTkButton):
    ...
    def __init__(self, parent, text="", variant="secondary",
        ...
        kwargs.setdefault("corner_radius", _BUTTON_CORNER_RADIUS)
